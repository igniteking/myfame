        ...
        if ($resultCheck < 1) {
            ...
            if ($row = mysqli_fetch_assoc($result)) {
                ...
                if ($hashedPwdCheck == false) {
                    ...
                    echo "<meta http-equiv=\"refresh\" content=\"0; url=../index.php?login=1\">";
                    exit();
                }
            } else {
                echo "ERROR";
...
function Register($user_type, $username, $password, $r_pswd, $conn, $email, $code)
{
                    ...
                    if (preg_match("/[A-Z]/", $password)) {
                        if (preg_match("/[a-z]/", $password)) {
                            if (preg_match("/\W/", $password)) {
                                ...
                                $mycode = rand(0, 1000);
                                mysqli_query($conn, "INSERT INTO `user_data`(`id`, `user_name`, `user_email`, `user_password`, `user_type`, `profile_picture`, `created_at`, `mycode` , `code`) VALUES 
                                (NULL,'$username','$email','$hashedPwd','$user_type','','$date', '$mycode', '$code')");
                                $get_id = Fetch_single_row($conn, "SELECT id FROM user_data WHERE user_email = '$email'");
                                mysqli_query($conn, "INSERT INTO `main_wallet`(`id`, `user_id`, `amount`) VALUES (NULL,'$get_id','0')");
                                mysqli_query($conn, "INSERT INTO `refral_wallet`(`id`, `user_id`, `amount`) VALUES (NULL,'$get_id','0')");
